...
import { ageLabel, loadCachedWeek, loadWeekOrCached, saveCachedWeek } from "./timetable-cache";
...
describe("mentett hét", () => {
  ...
  test("oda-vissza, alanyonként és hetenként", () => {
    ...
    expect(loadCachedWeek("12A", "2026-09-14")?.view.subject?.short).toBe("12A");
  ...
  test("legfeljebb négy bejegyzés, a legrégebbi esik ki", () => {
    ...
    ["2026-09-07", "2026-09-14", "2026-09-21", "2026-09-28", "2026-10-05"].forEach((w, i) => {
    ...
    expect(Object.keys(JSON.parse(b.localStorage.getItem("orarend:week-cache:v1") as string))).toHaveLength(4);
  });
  ...
  test("régi formátum: hiányzó mezők alapértékkel, a régi resolvedClass-ból alany", () => {
    const legacy = {
      ...
      days: [{ name: "Hétfő", dateKey: "2026-09-14", dateLabel: "", week: "A", dayOfWeek: 1, isToday: false }],
    };
    ...
    expect(cached?.view.days[0]).toMatchObject({ teaching: null, notes: [], bells: null });
  });

  test("sérült tár: null", () => {
    b.localStorage.setItem("orarend:week-cache:v1", JSON.stringify({ "12A|2026-09-14": { view: {} } }));
    expect(loadCachedWeek("12A", "2026-09-14")).toBeNull();
...
describe("loadWeekOrCached", () => {
  ...
  const offline = view({ ok: false, error: { kind: "network", title: "", message: "", retryable: true } });
  const unknown = view({ ok: false, error: { kind: "unknown-class", title: "", message: "", retryable: false } });

  test("friss siker: elmenti, nem jelöli mentettnek", async () => {
    const result = await loadWeekOrCached("12A", "2026-09-14", async () => view());
  ...
  test("elérhetetlen forrás: a mentett példány", async () => {
    saveCachedWeek("12A", "2026-09-14", view());
    const result = await loadWeekOrCached("12A", "2026-09-14", async () => offline);
  ...
  test("nem hálózati hiba: a hiba marad, nincs tartalék", async () => {
    saveCachedWeek("12A", "2026-09-14", view());
    const result = await loadWeekOrCached("12A", "2026-09-14", async () => unknown);
    expect(result.cached).toBeNull();
  ...
  test("dobó lekérés: mentett példány, vagy továbbdobás", async () => {
    ...
    await expect(loadWeekOrCached("12A", "2026-09-14", boom)).rejects.toThrow("boom");
    saveCachedWeek("12A", "2026-09-14", view());
    expect((await loadWeekOrCached("12A", "2026-09-14", boom)).cached).not.toBeNull();
  });
