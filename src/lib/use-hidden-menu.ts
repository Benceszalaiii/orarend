...
//*
//! KISZOLGÁLÓN ÉS AZ ELSŐ KÉPKOCKÁN A `DEFAULT_HIDDEN_MENU` LÁTSZIK — ez nem
//! kompromisszum, hanem a helyes alapértelmezés. A hidratálás így nem talál
//! eltérést (ugyanaz a minta, mint a `useStoredIdentity`-nél), villanás pedig
//! nincs: telefonon a lap csukva születik, a sáv-alak (`useWideChrome`) pedig
//! szintén csak a csatolás után jelenik meg — mire bármelyik sor kirajzolódik,
//! ez a horog már a tárolt (vagy az alapértelmezett) listán áll.
//! ═══════════════════════════════════════════════════════════════════════════
...
export function useHiddenMenu(): HiddenMenu {
  ...
  const toggle = useCallback((id: MenuItemId) => {
    ...
    const next = new Set(loadHiddenMenu() ?? DEFAULT_HIDDEN_MENU);
    if (next.has(id)) next.delete(id);
    else next.add(id);
    saveHiddenMenu([...next]);
  }, []);

  const reset = useCallback(() => saveHiddenMenu([]), []);
