...
import { forgetSyncState, isApplyingSyncedPrefs, syncPrefs } from "./prefs-sync";
...
describe("syncPrefs", () => {
  ...
  test("üres készülék: a szerver beállításai jönnek le, feltöltés nélkül", async () => {
    onGet = () => json({ prefs: { ...EMPTY_PREFS, class: "12A" }, revision: 4, updatedAt: "2026-09-01T00:00:00Z" });
    expect(await syncPrefs("u1")).toEqual({ status: "ok", revision: 4, updatedAt: "2026-09-01T00:00:00Z" });
  ...
  test("első szinkron ezen a fiókon: a helyi nyer, és felmegy", async () => {
    saveCachedClass("10B");
    onGet = () => json({ prefs: { ...EMPTY_PREFS, class: "12A", theme: "dark" }, revision: 2, updatedAt: null });
    onPut = [(init) => json({ prefs: JSON.parse(init?.body as string).prefs, revision: 3, updatedAt: "x" })];
    expect(await syncPrefs("u1")).toEqual({ status: "ok", revision: 3, updatedAt: "x" });
    expect(putBody(0)).toMatchObject({ baseRevision: 2, prefs: { class: "10B", theme: "dark" } });
  ...
  test("a szerver újabb a legutóbbi szinkronnál: a távoli nyer", async () => {
    ...
    onGet = () => json({ prefs: { ...EMPTY_PREFS, class: "12A" }, revision: 5, updatedAt: null });
    onPut = [(init) => json({ prefs: JSON.parse(init?.body as string).prefs, revision: 6 })];
  ...
  test("más fiók jelölője nem számít", async () => {
    ...
    onGet = () => json({ prefs: { ...EMPTY_PREFS, class: "12A" }, revision: 5 });
    onPut = [(init) => json({ prefs: JSON.parse(init?.body as string).prefs, revision: 6 })];
    await syncPrefs("u1");
  ...
  test("ütközés: a friss szerverállapotra fésül, és egyszer újrapróbál", async () => {
    ...
    onPut = [
      () => json({ current: { prefs: { ...EMPTY_PREFS, theme: "light" }, revision: 7 } }, { status: 409 }),
      (init) => json({ prefs: JSON.parse(init?.body as string).prefs, revision: 8, updatedAt: "y" }),
    ];
    expect(await syncPrefs("u1")).toEqual({ status: "ok", revision: 8, updatedAt: "y" });
    expect(putBody(1)).toMatchObject({ baseRevision: 7, prefs: { class: "10B", theme: "light" } });
  ...
  test("a sérült szerverválasz üres beállításnak számít", async () => {
    ...
    expect(await syncPrefs("u1")).toEqual({ status: "ok", revision: 0, updatedAt: null });
  });

  test("a szinkron alkalmazása alatt a jelző igaz, utána hamis", async () => {
    const seen: boolean[] = [];
    b.window.addEventListener("orarend:prefs-changed", () => seen.push(isApplyingSyncedPrefs()));
    onGet = () => json({ prefs: { ...EMPTY_PREFS, class: "12A" }, revision: 1 });
    await syncPrefs("u1");
